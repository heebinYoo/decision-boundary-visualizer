...
def main(arg_data_name=None):
    ...
    plt.scatter(train_embedding_list[np.where(train_label_list == seen[0]), 0], train_embedding_list[np.where(train_label_list == seen[0]), 1], c='r', alpha=0.1, s=100, label='seen1')
    plt.scatter(train_embedding_list[np.where(train_label_list == seen[1]), 0], train_embedding_list[np.where(train_label_list == seen[1]), 1], c='b', alpha=0.1, s=100, label='seen2')
    plt.scatter(test_embedding_list[np.where(test_label_list == unseen[0]), 0], test_embedding_list[np.where(test_label_list == unseen[0]), 1], c='g', alpha=0.5, s=5, label='unseen1')
    plt.scatter(test_embedding_list[np.where(test_label_list == unseen[1]), 0], test_embedding_list[np.where(test_label_list == unseen[1]), 1], c='y', alpha=0.5, s=5, label='unseen2')
    plt.legend()

    #plt.show()

    writer.add_figure('mnist-embedding-space', fig, num_epochs)
